...
$stmt = $db->prepare("SELECT COUNT(*) AS allcount FROM pneus LEFT JOIN usuarios ON pneus.usuario = usuarios.idusuarios WHERE uso = 1 ");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = $db->prepare("SELECT COUNT(*) AS allcount FROM pneus LEFT JOIN usuarios ON pneus.usuario = usuarios.idusuarios WHERE uso = 1 AND 1 ".$searchQuery);
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $db->prepare("SELECT * FROM pneus LEFT JOIN usuarios ON pneus.usuario = usuarios.idusuarios WHERE uso = 1 AND 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
