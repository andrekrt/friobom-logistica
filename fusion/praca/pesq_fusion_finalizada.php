...
$stmt = $db->prepare("SELECT COUNT(*) AS allcount FROM fusion_praca LEFT JOIN veiculos ON fusion_praca.veiculo = veiculos.cod_interno_veiculo LEFT JOIN auxiliares_rota ON fusion_praca.ajudante = auxiliares_rota.idauxiliares LEFT JOIN rotas ON fusion_praca.rota = rotas.cod_rota LEFT JOIN usuarios ON fusion_praca.usuario = usuarios.idusuarios WHERE situacao = 'Finalizado'");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = $db->prepare("SELECT COUNT(*) AS allcount FROM fusion_praca LEFT JOIN veiculos ON fusion_praca.veiculo = veiculos.cod_interno_veiculo LEFT JOIN auxiliares_rota ON fusion_praca.ajudante = auxiliares_rota.idauxiliares LEFT JOIN rotas ON fusion_praca.rota = rotas.cod_rota LEFT JOIN usuarios ON fusion_praca.usuario = usuarios.idusuarios WHERE 1 AND situacao = 'Finalizado'".$searchQuery);
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $db->prepare("SELECT * FROM fusion_praca LEFT JOIN veiculos ON fusion_praca.veiculo = veiculos.cod_interno_veiculo LEFT JOIN auxiliares_rota ON fusion_praca.ajudante = auxiliares_rota.idauxiliares LEFT JOIN rotas ON fusion_praca.rota = rotas.cod_rota LEFT JOIN usuarios ON fusion_praca.usuario = usuarios.idusuarios WHERE 1 AND situacao ='Finalizado' ".$searchQuery. "  ORDER BY idfusion_praca DESC LIMIT :limit,:offset");
