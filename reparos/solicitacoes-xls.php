...
if (isset($_SESSION['idUsuario']) && empty($_SESSION['idUsuario']) == false && ($result>0)  ) {
    ...
    $sql = $db->query("SELECT token, data_atual, monthname(data_atual), YEAR(data_atual), placa, veiculos.categoria as categoriaVeiculo, motorista, rota, problema, local_reparo, peca_reparo.categoria as categoriaPeca, descricao, qtd, un_medida, vl_unit, vl_total, frete, num_nf, nome_usuario, situacao, data_aprovacao, obs FROM solicitacoes_new LEFT JOIN peca_reparo ON solicitacoes_new.peca_servico = peca_reparo.id_peca_reparo LEFT JOIN usuarios ON solicitacoes_new.usuario = usuarios.idusuarios LEFT JOIN veiculos ON solicitacoes_new.placa = veiculos.placa_veiculo");
