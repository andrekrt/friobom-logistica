...
if(isset($_SESSION['idUsuario']) && empty($_SESSION['idUsuario'])==false && $_SESSION['tipoUsuario'] == 3 || $_SESSION['tipoUsuario'] == 99){
    ...
    if(isset($_GET['id']) && empty($_GET['id'])==false){
        ...
        $consultaOrdem = $db->query("SELECT * FROM solicitacoes WHERE id = $idOrdem ");
        if($consultaOrdem){
            $dados = $consultaOrdem->fetch();

            $veiculo = $dados['placarVeiculo'];
            $servico01 = $dados['servico']; 
            $valor01 = $dados['valor'];
        ...
        <div class="container-fluid corpo">
            ...
            <div class="tela-principal">
                <div class="menu-superior">
                   ...
                   <div class="title">
                        <h2>Solicitação Secundária</h2>
                   </div>
                </div>
                <!-- dados exclusivo da página-->
                <div class="menu-principal">
                    <form action="add-novo-servico.php" class="despesas" method="post">
                        <input type="hidden" name="id" value="<?php echo $idOrdem?>">
                        <div class="form-row">
                            <div class="form-group col-md-4 espaco">
                                <label for="">Placa Veículo</label>
                                <input type="text" readonly name="placa" class="form-control" value="<?php echo $veiculo ?>">
                            </div>
                            <div class="form-group col-md-4 espaco">
                                <label for="">Primeira Peça/Serviço</label>
                                <input type="text" readonly name="servico01" class="form-control" value="<?php echo $servico01 ?>">
                            </div>
                            <div class="form-group col-md-4 espaco">
                                <label for="">Valor Adicional</label>
                                <input type="text" readonly name="valor01" class="form-control" value="<?php echo $valor01 ?>">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-6 espaso">
                                <label for=""> Novo Serviço/Peça </label>
                                <input type="text" required name="servico02" class="form-control">
                            </div>
                            <div class="form-group col-md-6 espaso">
                                <label for=""> Descrição </label>
                                <input type="text" name="descricao02" class="form-control">
                            </div>
                        </div>
