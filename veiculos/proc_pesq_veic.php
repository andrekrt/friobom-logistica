...
$stmt = $db->prepare("SELECT COUNT(*) AS allcount FROM veiculos WHERE ativo = 1");
$stmt->execute();
$records = $stmt->fetch();
...
## Fetch records
$stmt = $db->prepare("SELECT * FROM veiculos WHERE 1 AND ativo = 1 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
